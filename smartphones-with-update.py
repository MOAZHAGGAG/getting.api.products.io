...
async def save_to_postgresql(products):
    ...
    If a product with the same link exists, update it; otherwise, insert it.
    """
    try:
        ...
        async with conn.transaction():
            for product in products:
                # Check if the product with the same link already exists in the database
                existing_product = await conn.fetchrow(
                    "SELECT * FROM productstestt WHERE link = $1", product['link']
                )

                if existing_product:
                    # If the product exists, update it
                    await conn.execute("""
                        UPDATE productstestt
                        SET name = $1, specs = $2, new_price = $3, old_price = $4, 
                            brand = $5, category = $6, datetime = $7
                        WHERE link = $8
                    """, product['name'], product['specs'], product['new_price'], product['old_price'],
                       product['brand'], product['category'], product['timestamp'], product['link'])
                    # print(f"Updated product: {product['link']}")
                else:
                    ...
                    """, product['name'], product['specs'], product['new_price'], product['old_price'],
                       product['link'], product['brand'], product['category'], product['timestamp'])
                    # print(f"Inserted new product: {product['link']}")
